...
def predict_category(text,loaded_model = False):
    ...
    categories = ['price','app','delivery','pros','general','service']
...
arabic_ds = pd.read_excel('/home/ahmed/coding/NLPprojects/TransformerClassification/main/output.xlsx')
arabic_ds = arabic_ds[:1000]
...
x_train, x_test, y_train, y_test = train_test_split(arabic_ds['text'].to_list(), arabic_ds['category'].to_list(), test_size=0.3, random_state=seed)
...
model.save_weights('nlp_project_category2.h5') # save the trained weights

saved_model = get_model(max_len) # crearte new model
saved_model.load_weights('nlp_project_category2.h5') # load the weights


predicted_category = predict_category("من اسوء التطبيقات وكمان المفروض الزبون علي حق لا هما عكس المطعم عل حق ويولع الزبون الاكل يجيلو زباله يجيلو في عيب والحاجه الي ترنفز بجد يخلو مسئوليتهم يا اما ننتظر عدة ايام علشان تتحل المشكله واكن فلوسنا حرام وحلال ليهم",loaded_model = True)
